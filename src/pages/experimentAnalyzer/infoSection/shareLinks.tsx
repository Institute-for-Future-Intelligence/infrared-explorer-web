...
const ShareLinks = ({ title = 'Infrared Explorer' }: ShareLinkProps) => {
  ...
  // staying compact enough to share one row with the rating stars. No negative row margin: this row is
  // the rightmost child of the space-between ActionBar, so a -2px right margin pushed its border box
  // 2px past the panel edge and triggered a horizontal scrollbar on the whole info panel. The 2px
  // inset from dropping it is imperceptible.
  const buttonStyle = { display: 'inline-flex', padding: 2, lineHeight: 0 };
  const rowStyle = { display: 'flex', flexWrap: 'wrap' as const, alignItems: 'center', gap: 2 };
